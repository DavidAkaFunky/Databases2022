CREATE VIEW vendas(ean, cat, ano, trimestre, dia_mes, dia_semana, distrito, concelho, unidades) AS
    SELECT ean, name AS cat, EXTRACT(YEAR from instante) AS ano, EXTRACT(QUARTER from instante) AS trimestre, EXTRACT(DAY from instante) AS dia_mes, EXTRACT(WEEKDAY from instante) AS dia_semana, distrito, concelho, unidades
